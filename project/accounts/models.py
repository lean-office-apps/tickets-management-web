...
class RecordStatus(enum.Enum):
    ...
    """
    ACTIVE = "Active"  # Represents an editable record in the database
    DELETED = "Delete"  # Represents a deleted record that nothing can be done on it in the database
    ACTIVE_LOCKED = "Active Locked"  # Represents a readonly record
...
class LookUp(BaseEntity):
    ...
    category = models.CharField(null=True, blank=True,
                                ...
                                choices=[(record.name, record.value) for record in LookUpCategory],
                                verbose_name='LookUp')
...
class CustomUser(AbstractBaseUser, PermissionsMixin, BaseEntity):
    ...
    gender = models.ForeignKey(LookUp, on_delete=models.PROTECT, null=True, blank=True,
                               related_name="%(app_label)s_%(class)s_gender",
                               related_query_name="%(app_label)s_%(class)ss_gender")
